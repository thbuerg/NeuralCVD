...
from prefect.engine.serializers import PandasSerializer, JSONSerializer
...
output_directory = '/data/analysis/ag-reils/ag-reils-shared/cardioRS/data/3_datasets_post'
output_name = '210616_centres_dask'

# pd_serializer = PandasSerializer(file_type='csv')
...
class ApplyImputer(pf.Task):
    ...
    def run(self, partition_split_dict):
        ...
        #data.reset_index().to_csv(f"{data_output_path}/datam.csv")
        ...
        #data_imputed.reset_index().to_csv(f"{data_output_path}/data_imputed.csv")
...
class ApplyNorm(pf.Task):
    ...
    def run(self, partition_split_dict):
        ...
        #partition_split_dict['data'].reset_index().to_csv(f"{data_output_path}/data_imputed_normalized.csv")
...
def read_and_merge_data(covariate_paths, input_data_dir):
    ...
    #description_merged.reset_index().to_csv(f"{output_path}/description.csv")

    return {"data": data_merged, "description": description_merged}

@pf.task(name="encode_categoricals",
      target="data_encoded.p",
      checkpoint=True,
      result=LocalResult(dir=f"{output_directory}/{output_name}")#, serializer=pd_serializer)
      )
def encode_categoricals(data_dict):
    ...
    #data.reset_index().to_feather(f"{output_directory}/{output_name}/data_encoded.feather")
...
def apply_exclusion_criteria(data_dict, exclusion_criteria):
    ...
    #data_excl.reset_index().to_csv(f"{output_path}/data_excl.csv")
...
def fit_imputer(partition_dict):
    ...
    data = partition_dict['data'].loc[eids_train]#.query('NMR_FLAG==True')
    partition = partition_dict["cv_partition"]

    missing = data.columns[data.isna().any()].to_list()
    missing = [col for col in missing if not "NMR_" in col]

    variable_schema = {}
    for m in missing:
        variable_schema[m] = [x for x in missing if x != m]+["MACE_event", "MACE_event_time"]
    ...
    kernel.mice(3, n_jobs=20, n_estimators=8,
...
with pf.Flow("ukb_pipeline") as flow:
    input_data_dir = pf.Parameter('input_data',
                                  default='/data/analysis/ag-reils/ag-reils-shared/cardioRS/data/2_datasets_pre/210514_metabolomics/')
    #exclusion_criteria = pf.Parameter('exclusion_criteria',
    #                               default="myocardial_infarction == False & stroke == False & statins == False")
    ...
    #data_dict = apply_exclusion_criteria(data_dict, exclusion_criteria)
...
if __name__ == "__main__":
    flow.executor = LocalDaskExecutor(scheduler="threads", num_workers=80)
    #flow.executor = DaskExecutor(cluster_kwargs={"n_workers": 10, "threads_per_worker": 20}, debug=True)
    ...
    # run in the cloud
    # flow.register(project_name="ukbb_pipeline")
    # flow.run()