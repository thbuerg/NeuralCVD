...
def cox_ph_loss(logh, durations, events, eps=1e-7):
    ...
    if events.sum() > 0:
        ...
        pll = - logh.sub(log_cumsum_h).mul(events).sum()
    return pll
...
def OLD_DSM_uncensored_loss(logf_ts, ks, events, e=1):
    """
    We minimize the ELBO of log P(DATASET_uncensored)
    equalling the negative sum over all log hazards.
    inputs are expected to be 2D Tensors of shape [B, k_dim]
    ...
    elbo = logf_ts.add(F.log_softmax(ks, dim=1)).sum(1, keepdim=True) # ks and f_ts are of shape [B, kdim, nevents]
    mask = torch.eq(events, e_)
    elbo = elbo[mask]
...
def OLD_DSM_censored_loss(logS_ts, ks, events, e=1):
    """
    NLL on log hazards.
    ...
    elbo = logS_ts.add(F.log_softmax(ks, dim=1)).sum(1, keepdim=True) # ks and S_ts are of shape [B, kdim, nevents]
    mask = torch.ne(events, e_)
    elbo = elbo[mask]
